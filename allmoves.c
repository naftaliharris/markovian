...
{
	return(
		BishopMoves[loc][magictransform((pos->allpieces & BishopMasks[loc]), BishopMagic[loc], BishopMagicSize[loc])]
	);
}
...
{
	return(
		RookMoves[loc][magictransform((pos->allpieces & RookMasks[loc]), RookMagic[loc], RookMagicSize[loc])]
	);
}
...
inline uint64_t is_check(struct position *pos){
	...
	uint64_t result = (
		(bishopmoves(pos, king_pos) & (pos->pieces[wbishops_n | pos->towait] | pos->pieces[wqueens_n | pos->towait]))	| //bishop and queen checks
...
inline void bishop_move(struct position *pos, struct move_array *m, unsigned char bishops)
{
