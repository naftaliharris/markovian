...
{
	return (BishopMoves[loc]
		[magictransform
		 ((pos->allpieces & BishopMasks[loc]), BishopMagic[loc],
		  BishopMagicSize[loc])]
	    );
}
...
{
	return (RookMoves[loc]
		[magictransform
		 ((pos->allpieces & RookMasks[loc]), RookMagic[loc],
		  RookMagicSize[loc])]
	    );
}

//checks whether king is in check, 0 if no, not 0 if yes
//this might be better suited as a macro so that don't need to compute entire thing if earlier evals to true (?)
...
inline uint64_t is_check(struct position *pos)
{
	...
	uint64_t result = ((bishopmoves(pos, king_pos) & (pos->pieces[wbishops_n | pos->towait] | pos->pieces[wqueens_n | pos->towait])) |	//bishop and queen checks
			   (knightmoves[king_pos] & pos->pieces[wknights_n | pos->towait]) |	//knight checks
			   (rookmoves(pos, king_pos) & (pos->pieces[wrooks_n | pos->towait] | pos->pieces[wqueens_n | pos->towait])) |	//rook and queen checks
			   (pawn_attacks[pos->towait][king_pos] & pos->pieces[wpawns_n | pos->towait])	//pawn checks
	    );
	return (result);
}

void rook_move(struct position *, struct move_array *, unsigned char);
inline void rook_move(struct position *pos, struct move_array *m,
		      unsigned char rooks)
{
...
inline void knight_move(struct position *pos, struct move_array *m,
			unsigned char knights)
{
	...
	for (int i = 0; i < 10; i++) {	// can have up to 10 knights of same color on the board, (for loop for compiler loop unrolling)
		if ((index_from = ffsll(knight_pos)) != 0) {
			index_from--;
			moves =
			    knightmoves[index_from] & ~pos->
			    sumpieces[knights & COLOR];
			for (int i = 0; i < 8; i++) {
				if ((index_to = ffsll(moves)) != 0) {
					index_to--;
					add_move(m, index_from, index_to,
...
inline void king_move(struct position *pos, struct move_array *m,
		      unsigned char king)
{
...
inline void pawn_move(struct position *pos, struct move_array *m,
		      unsigned char pawns)
{
	...
	case WHITE:
		...
		for (int i = 0; i < 8; i++) {
			if ((index_to = ffsll(moves)) != 0) {
				index_to--;
				add_move(m, index_to + S, index_to, pawns,
					 nopiece_n);
				moves &= notlinboard[index_to];
			} else {
				break;
		...
		moves =
		    moveN(moveN(pawn_pos) & ~pos->allpieces) & ~pos->
		    allpieces & rank[3];
		for (int i = 0; i < 8; i++) {
			if ((index_to = ffsll(moves)) != 0) {
				index_to--;
				add_move_forward2(m, index_to + S + S, index_to,
						  pawns, nopiece_n);
				moves &= notlinboard[index_to];
			} else {
				break;
	...
	default:		//black
		...
		for (int i = 0; i < 8; i++) {
			if ((index_to = ffsll(moves)) != 0) {
				index_to--;
				add_move(m, index_to + N, index_to, pawns,
					 nopiece_n);
				moves &= notlinboard[index_to];
			} else {
				break;
		...
		pawn_pos = pos->pieces[pawns];
		moves =
		    moveS(moveS(pawn_pos) & ~pos->allpieces) & ~pos->
		    allpieces & rank[4];
		for (int i = 0; i < 8; i++) {
			if ((index_to = ffsll(moves)) != 0) {
				index_to--;
				add_move_forward2(m, index_to + N + N, index_to,
						  pawns, nopiece_n);
				moves &= notlinboard[index_to];
			} else {
				break;
